...
import spider65.ebike.tsdz2_sw102.utils.Cramer;
import spider65.ebike.tsdz2_sw102.utils.LinearRegression;
import spider65.ebike.tsdz2_sw102.utils.RollingAverage;
import spider65.ebike.tsdz2_sw102.utils.Utils;

import static spider65.ebike.tsdz2_sw102.TSDZConst.CMD_MOTOR_DATA;
import static spider65.ebike.tsdz2_sw102.TSDZConst.TEST_START;
import static spider65.ebike.tsdz2_sw102.TSDZConst.TEST_STOP;
...
public class HallCalibrationActivity extends AppCompatActivity {
    ...
    private boolean calib_started = true;
    private int msgCounter;
    private int step = 0;
    private final byte[] dutyCycles = {35, 75, 105, (byte)(145 & 0xff)};

    //private final byte[] dutyCycles = {30, 70, 100, (byte)(150 & 0xff)};
    //private final byte[] dutyCycles = {30, 60, 90, (byte)(140 & 0xff)};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mIntentFilter.addAction(TSDZBTService.TSDZ_MOTOR_DATA_BROADCAST);
    ...
    protected void onStop() {
        if (calibrationRunning && TSDZBTService.getBluetoothService() != null)
            TSDZBTService.getBluetoothService().writeCommand(new byte[] {CMD_MOTOR_DATA,TEST_STOP, 0x00});
    ...
    private void startCalib() {
        ...
        for (RollingAverage ra: avg)
            ra.reset();
        TSDZBTService.getBluetoothService().writeCommand(new byte[] {CMD_MOTOR_DATA, TEST_START, dutyCycles[step]});
    }
            ...
            switch (intent.getAction()) {
                ...
                case TSDZBTService.TSDZ_MOTOR_DATA_BROADCAST:
                    ...
                    byte[] data = intent.getByteArrayExtra(TSDZBTService.VALUE_EXTRA);
                     Log.d(TAG,"TSDZ_MOTOR_DATA_BROADCAST: " + Utils.bytesToHex(data));
                    if (data[1] == TEST_START) {
                    ...
                    long sum = 0;
                    for (int i = 0; i < 6; i++) {
                        avg[i].add(((data[i * 2 + 3] & 255) << 8) + (data[i * 2 + 2] & 255));
