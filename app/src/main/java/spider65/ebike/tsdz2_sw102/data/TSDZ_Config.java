...
public class TSDZ_Config  {
    ...
    public int ui8_time_field_enable;
    ...
    public int ui8_lcd_power_off_time_minutes;
    public int ui8_units_type;
    public int ui8_energy_saving_mode_level;
    public int  ui8_plus_long_press_switch;
    ...
    private boolean sixth_data_chunk = true;
    ...
    public boolean setData(byte[] data) {
        ...
        if (data[1] == 1) {
            ...
            ui8_time_field_enable = (data[14] & 255);
            ui8_motor_current_min_adc = (data[15] & 255);
            ui8_wheel_max_speed = (data[16] & 255);
            ui32_wh_x10_100_percent = (data[17] & 255) + ((data[18] & 255) << 8)  + ((data[19] & 255) << 16);
            return false;
        }else if(data[1] == 3) {
            for (int i = 0; i < 5; i++)
            ...
            ui32_wh_x10_offset = (data[17] & 255) + ((data[18] & 255) << 8)  + ((data[19] & 255) << 16);
            return false;
        }else if(data[1] == 4) {
            for (int i = 0; i < 5; i++)
            ...
            for (int i = 0; i < 5; i++)
                ...
                ui8_lcd_power_off_time_minutes = (data[12] & 255);
                ui8_units_type = (data[13] & 255);
                ui8_energy_saving_mode_level = (data[14] & 255);
                ui8_plus_long_press_switch = (data[15] & 255);
             //16-19 free slots
            return false;
        }else if(data[1] == 5) {
            for (int i = 0; i < 6; i++)
            ...
            for (int i = 0; i < 6; i++)
                ui8_hall_ref_angles[i] = (data[2+i] & 255);
            for (int i = 0; i < 6; i++)
                ui8_hall_counter_offset[i] = (data[8+i] & 255);
        }
        return true;
    }
    ...
    public byte[] toByteArray() {
        ...
        if(first_data_chunk) {
            ...
            data[14] = (byte) ui8_time_field_enable;
            data[15] = (byte) ui8_motor_current_min_adc;
            data[16] = (byte) ui8_wheel_max_speed;
            data[17] = (byte) ui8_energy_saving_mode_level;
            data[18] = (byte) ui8_plus_long_press_switch;
            ...
            data[18] = (byte) ui8_lcd_power_off_time_minutes;
            data[19] = (byte) (ui8_units_type & 0xff);

            return data;

        }else if (fifth_data_chunk)  {

            fifth_data_chunk = false;
        ...
        }else if (sixth_data_chunk)  {

            data[0] = 0x53;
            data[1] = 6;                            //sixth chunk of config data

            for (int i = 0; i < 6; i++)
                data[2+i] = (byte)(ui8_hall_ref_angles[i]);
            for (int i = 0; i < 6; i++)
                data[8+i] = (byte)(ui8_hall_counter_offset[i]);

            first_data_chunk = true;
            second_data_chunk = true;
            third_data_chunk = true;
            fourth_data_chunk = true;
            fifth_data_chunk = true;
        }
        return data;
    }
}
